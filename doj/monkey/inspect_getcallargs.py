# -*- coding: utf-8 -*-
"""
Jython 2.7b2 is missing the `inspect.getcallargs` function, so we
add it here if it is not existing.

Refers to bug: http://bugs.jython.org/issue2184
"""
...
def install():
    # adding the original function if not existing
    if not getattr(inspect, 'getcallargs', None):
